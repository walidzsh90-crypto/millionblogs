# Architecture Freeze v2 — MillionBlogs Backend
...
|-----------|:------------:|----------------|
| **Architecture** | **7.5** | Well-modularized with clear layer separation. Some boundary violations and Global-module coupling remain. |
| **Maintainability** | **7.0** | Consistent patterns improved by fixing encapsulation violation. Raw SQL in repositories and no base abstractions remain. |
...
| **Technical Debt** | **5.5** | 30 performance findings, 22 security findings (all critical/high fixed), 3 architectural fixes applied (hold/release, search params, encapsulation). |

**Composite Health: 6.4 / 10** — Improved from 6.0 by fixing the 3 most impactful architectural weaknesses. Critical data integrity issues resolved.
...
| Raw SQL via `$queryRawUnsafe` | `SearchRepository` | Bypasses Prisma type safety |
| ~~Direct `['prisma']` access~~ | ~~AuthService~~ | **FIXED** — see 3.2 |

### 3.2 ~~Private Member Access Violation~~ **FIXED**

`AuthService` previously accessed `sessionsRepository['prisma']` via bracket notation to perform password reset operations. This broke encapsulation.

**Fix applied (v2):** Four new methods were added to `SessionsRepository` — `findRecentPasswordReset()`, `createPasswordReset()`, `findPasswordResetByToken()`, `markPasswordResetAsUsed()`. `AuthService` now uses these methods instead of direct `prisma` access.
...
### 9.1 Architecture Score: 8.0/10

**Strengths:**
- Optimistic locking with `version` column on all balance-changing operations
- `$transaction` wrapping for atomic credit/debit (and now hold/release)
- Idempotency keys prevent double-processing
- Full audit trail via WalletTransaction (immutable ledger)
- Separate `purchasedBalance` and `bonusBalance` tracking

**Weaknesses:**
- ~~`hold()`/`release()` did not modify balance~~ **FIXED** — see 9.2
- `getWalletWithLock()` does not actually lock (misleading method name)
- No balance reclamation for expired holds
- `refund()` restores to purchasedBalance only (ignores original split)

### 9.2 ~~Hold/Release Balance Locking~~ **FIXED**

**Previous code (v1):**
- `hold()` checked balance but didn't deduct it — allowed overspend
- `release()` created a transaction record but didn't restore wallet balance
- Neither used `$transaction`, so a concurrent operation could observe stale state
- `ConflictException` was not imported — the version conflict path would crash

**Fix applied (v2):**
- `hold()` is now wrapped in `$transaction` — reads wallet, deducts from purchased/bonus, updates with version check, records hold transaction with correct `balanceAfter`. Emits `WALLET_DEBITED` event.
- `release()` is now wrapped in `$transaction` — reads wallet with version check, restores to purchased balance, records release transaction with correct `balanceAfter`. Emits `WALLET_CREDITED` event.
...
### 11.1 Architecture Score: 6.5/10
...
**Weaknesses:**
- ~~**Critical:** Parameter index mismatch in raw SQL~~ **FIXED** — see 11.2
...
### 11.2 ~~Search Parameter Index Mismatch~~ **FIXED**

**Previous code (v1):**
- `orderClause` used `$${paramIndex - (searchTerm ? 1 : 0)}` to reference the tsquery parameter
- When searchTerm was absent: computed `$0` (invalid PostgreSQL)
- When searchTerm + language filter both present: computed `$2` (language), not `$1` (searchTerm)
- `LIMIT $${paramIndex}` and `ORDER BY $${paramIndex}` could conflict on the same index
- `searchBlogs()` had the same bug: `$${searchTerm ? 1 : 0}`

**Fix applied (v2):**
- Introduced `rankExpr` — computed once as either `ts_rank(..., plainto_tsquery('english', $1))` or `0::float4`
- Used consistently in SELECT, ORDER BY, and the tsquery expression
- Parameters always push searchTerm as `$1` (first element) when present, ensuring correct positional mapping
- Removed the fragile `paramIndex - (searchTerm ? 1 : 0)` arithmetic entirely
...
| 2 | ~~Raw SQL parameter index mismatch (search)~~ **FIXED** | — | — |
| 3 | ~~Hold/release doesn't lock balance~~ **FIXED** | — | — |
| 4 | ~~Private member access (`['prisma']`)~~ **FIXED** | — | — |
| 5 | FeatureAccessModule is `@Global()` | Circular dependency risk | Low |
...
### Critical (Blocking)
1. ~~Fix search parameter index mismatch (P-16)~~ **DONE**
2. ~~Remove private `['prisma']` access~~ **DONE**
...
5. ~~Fix hold/release to lock balance~~ **DONE**
...
|-----|------------------|--------|
| Wallet hold/release consistency | `src/wallet/wallet.service.ts` | `hold()` now deducts from balances inside `$transaction` with version check; `release()` restores balances inside `$transaction` with version check. Both emit appropriate events. |
| Search parameter binding | `src/search/search.repository.ts` | Introduced `rankExpr` variable (always `$1` for search term or `0::float4`). Removed fragile `paramIndex` arithmetic from ORDER BY. |
| AuthService encapsulation | `src/sessions/sessions.repository.ts`, `src/auth/auth.service.ts` | Added 4 password reset methods to `SessionsRepository`; replaced all `sessionsRepository['prisma']` calls with proper method calls. |

---

## 17. Conclusion

The codebase has a **solid foundation** with consistent patterns, clear layering, and well-separated concerns. The three architectural fixes in v2 resolve the most impactful data integrity and encapsulation issues:

1. **Hold/release now atomically deducts/restores balance** — no more overspend risk
2. **Search parameter indexing is now positionally correct** — no more incorrect search results
3. **AuthService uses proper repository methods** — no more private member access violation
...
3. **Technical debt accumulation** — raw SQL, no base abstractions, 12 missing indexes

**Architecture Health: 6.4/10** — Improved from 6.0. Critical data integrity issues resolved. Next focus should be event durability (outbox pattern) and performance optimization (caching, concurrency).

---

*See also: [ROADMAP_V2.md](./ROADMAP_V2.md) for implementation plans on outbox, caching, cursor pagination, analytics aggregation, and feed optimization.*
